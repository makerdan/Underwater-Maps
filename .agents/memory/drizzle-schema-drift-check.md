...
5. Keep the numeric migration prefix aligned with the journal index when accepting a generated migration; the pairing check uses the index to locate its snapshot.
6. When rebuilding a missing historical snapshot after a later snapshot already exists, `drizzle-kit generate` follows the highest snapshot file even if the journal is temporarily truncated. Generate in an isolated copy with later snapshots removed, or verify and repair `prevId` against the preceding snapshot; also compare the reconstructed schema state against the later snapshot with later migration fields removed.
