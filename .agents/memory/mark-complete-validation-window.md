...
## Stale-base code review attribution
The completion code review can diff against a base older than the latest merged task, attributing the *previous* merged task's files to your diff and rejecting. Verify with `git show <your commit> --stat` (and `git diff <parent> <commit> --name-only`); if your commit contains only your files, re-call markTaskComplete with `request_fresh_code_review: true` and explain the mis-attribution in skip_validation_reason. Also: completion validation runs EVERY registered validation command (all tiers + audit + palette e2e), not just the plan tier — attribute each baseline failure with a solo repro before finalizing.
