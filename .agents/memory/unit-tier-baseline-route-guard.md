...
- Legacy-plan gate steps (`check:failure-gate`, `fix:regression-guard-stubs`) scan ALL gitignored `.local/tasks/*.md` and can abort the tier on hundreds of old stubbed plans your diff never touched. Workaround: upsert a validation command running the same tier with `--skip check:failure-gate` (run-tier supports repeatable `--skip`); for regression-guard, sections with existing-but-noncompliant prose need a manual `**Self-satisfying**` prefix (usually only a handful of files). The 909-file **Why:** backfill is its own tracked task — never hand-fill it mid-task.
