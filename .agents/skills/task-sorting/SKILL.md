...
**Step 0-B — Drop non-PROPOSED refs.** Immediately after capturing the snapshot, iterate every ref in the working set and verify its current state. Remove from the working set any ref whose state is not `PROPOSED` (i.e. any ref whose state is `PENDING`, `IN_PROGRESS`, `IMPLEMENTED`, `MERGING`, `QUEUED`, `MERGED`, or `CANCELLED`). Log the drop count (e.g. "Dropped 3 non-PROPOSED tasks from working set"). These dropped refs are silently excluded for the remainder of the run — do not rename or mutate them in any way. If a task transitions out of PROPOSED mid-run (after the snapshot check), it is also silently skipped during any mutation step — never mutated. Record the drop count for the Phase 7 summary.
