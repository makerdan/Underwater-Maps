...
- api-client-react       - generated React Query hooks (TS project reference).
- api-zod                - generated Zod schemas.
- db                     - Drizzle schema, migration tooling, DB client.
- poe                    - Poe AI client wrapper.
- integrations/openai_ai_integrations - OpenAI integration helpers (server + React).
...
- lib/         - Zustand stores (settingsStore, uiStore, driftStore), drift physics
                 (computeDrift.ts), dev-auth bypass, test helpers.
...
Each artifact under artifacts/* registers itself with the Replit artifacts system (via its own artifact.toml) and gets a long-lived dev process managed by the workspace. In a fresh session they typically appear as:
  ...
  artifacts/bathyscan: web                              pnpm --filter @workspace/bathyscan run dev
...
=====================
  ...
  CLERK_PUBLISHABLE_KEY          API server    Yes (auth)      Used by the Clerk proxy middleware.
  CLERK_SECRET_KEY               API server    Yes (auth)      Server-side Clerk session verification.
  VITE_CLERK_PUBLISHABLE_KEY     Web app       Yes (auth)      Frontend Clerk SDK init. Without it, sign-in is disabled.
  VITE_CLERK_PROXY_URL           Web app       Optional        Override Clerk proxy URL (default: ${BASE_URL}clerk).
  ...
  VITE_DEV_AUTH_BYPASS           Web app       Dev/e2e only    When 1, bypasses Clerk in headless Playwright runs.
...
Top-level scripts (package.json):
  ...
  Command                                          Purpose
  -----------------------------------------------  --------------------------------------------------------
  pnpm run typecheck                               Codegen check + full TypeScript build across libs/artifacts.
