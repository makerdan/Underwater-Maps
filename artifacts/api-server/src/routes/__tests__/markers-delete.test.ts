...
vi.mock("@workspace/api-zod", async (importOriginal) => {
  ...
  return createApiZodMock(actual, {
    ...
    PatchUserCollectionsIdMetaBody: { safeParse: () => ({ success: false, error: { issues: [], message: "noop" } }) },
    PostUserCollectionsIdLayoutBody: { safeParse: () => ({ success: false, error: { issues: [], message: "noop" } }) },
