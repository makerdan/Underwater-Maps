...
const SKILL_ZIP_PATH = path.resolve(
  process.cwd(),
 ...
 *
 * Serves the failure-gate-skill.zip with Cache-Control: no-store so the
 * browser always pulls the latest copy regenerated by post-merge.sh.
 *
 * Access: auth-required; restricted to admin users (same rules as
 * /admin/bucket-monitor).
 */
router.get(
  "/admin/skill/failure-gate",
  requireAuth,
  asyncHandler(async (req, res) => {
    const userId = (req as AuthenticatedRequest).clerkUserId;
    ...
    if (!existsSync(SKILL_ZIP_PATH)) {
      res.status(404).json({ error: "not_found", details: "Skill zip not found on server" });
      return;
    }

    const fileBytes = readFileSync(SKILL_ZIP_PATH);
    res.set("Content-Type", "application/zip");
    res.set("Cache-Control", "no-store");
    res.set("Content-Disposition", 'attachment; filename="failure-gate-skill.zip"');
    res.send(fileBytes);
  }),
);
