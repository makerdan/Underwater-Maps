...
datasetDiscoveryRouter.delete("/datasets/presets/:id", requireAuth, asyncHandler(async (req, res): Promise<void> => {
  ...
  if (!isAdmin(userId)) {
    res.status(403).json({ error: "forbidden", details: "Administrator access is required." });
    return;
  }
...
datasetTerrainRouter.get("/datasets/:id/terrain", terrainFetchIpRateLimit, terrainFetchUserRateLimit, asyncHandler(async (req, res): Promise<void> => {
  ...
  if (CUSTOM_DATASET_UUID_RE.test(id) && !ALL_PRESET_DATASETS.some((d) => d.id === id)) {
    ...
    if (!stored.success) {
      const issues = stored.error.issues.map((i) => `${i.path.join(".") || "root"}: ${i.message}`).join("; ");
  ...
  const rawResolution = req.query["resolution"];
  const parsedResolution = rawResolution === undefined ? 256 : Number.parseInt(String(rawResolution), 10);
  const resolution = Number.isFinite(parsedResolution) ? Math.max(32, Math.min(512, parsedResolution)) : 256;
  ...
  try {
    ...
    if (!preview) {
      if (CUSTOM_DATASET_UUID_RE.test(id) && !ALL_PRESET_DATASETS.some((d) => d.id === id)) {
        ...
        const terrain = StoredTerrainJsonSchema.safeParse(row.terrainJson);
        if (!terrain.success) {
          const issues = terrain.error.issues.map((i) => `${i.path.join(".") || "root"}: ${i.message}`).join("; ");
        ...
        res.json(validateResponse(GetDatasetsIdPreviewResponse, {
          ...
          name: row.name,
          bbox: {
            minLon: terrain.data.minLon,
            minLat: terrain.data.minLat,
            maxLon: terrain.data.maxLon,
            maxLat: terrain.data.maxLat,
          },
          dataSource: terrain.data.dataSource === "gebco" ? "gebco" : "ncei",
...
datasetTerrainRouter.get("/datasets/:id/zones", asyncHandler(async (req, res): Promise<void> => {
  ...
  if (!isPreset) {
    ...
    if (CUSTOM_DATASET_UUID_RE.test(id)) {
      const rows = await db
        .select({ id: customDatasetsTable.id })
        .from(customDatasetsTable)
        .where(and(eq(customDatasetsTable.id, id), eq(customDatasetsTable.userId, callerId)));
      if (rows.length === 0) {
...
datasetIngestionRouter.post(
  ...
  asyncHandler(async (req: Request, res: Response): Promise<void> => {
    ...
    const ext = file.originalname.slice(file.originalname.lastIndexOf(".")).toLowerCase();
    const supportedMimeType = file.mimetype === "image/png" || file.mimetype === "image/jpeg";
    if (!supportedMimeType && ext !== ".png" && ext !== ".jpg" && ext !== ".jpeg") {
      res.status(415).json({
        error: "unsupported_file_type",
