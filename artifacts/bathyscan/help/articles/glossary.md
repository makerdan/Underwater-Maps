...
**Depth profile** — a vertical cross-section between two points on the terrain.
...
**Marker** — a persistent pin you drop on the seafloor.

**Orbit mode** — third-person camera that swings around a target. The default mode at load time.

**Overview map** — a small top-down map (press **O**) showing the whole dataset and your camera position.

**Paint mode** — a tool for manually correcting the AI's zone classification.
...
**Water type** — saltwater or freshwater. Determines available datasets, marker types, and AI vocabulary.

**Zone** — an AI-assigned label for a region of terrain (e.g. *sandy_shelf*, *aquatic_vegetation*).
