...
if (typeof HTMLCanvasElement !== "undefined") {
  ...
  HTMLCanvasElement.prototype.getContext = vi.fn(function(this: HTMLCanvasElement) {
    return {
      // Back-reference matching real CanvasRenderingContext2D.canvas so that
      // renderer helpers like renderContourLines can read ctx.canvas.width /
      // ctx.canvas.height from within the rAF loop without throwing.
      canvas: this ?? { width: 1024, height: 768 },
      fillStyle: "",
      fillRect: vi.fn(),
      clearRect: vi.fn(),
      beginPath: vi.fn(),
