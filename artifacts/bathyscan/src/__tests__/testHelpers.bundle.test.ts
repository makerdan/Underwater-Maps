...
describe("production bundle", () => {
  it("keeps the production bundle safe and split below the precache ceiling", async () => {
    ...
    const prevBasePath = process.env.BASE_PATH;
    ...
    // The registration module is generated by vite-plugin-pwa from the
    // configured base and scope. Assert the actual emitted source rather than
    // only config values: a trailing-slash regression previously generated
    // `//sw.js` and `scope: "//"` in production.
    const rootRegistrationSource = chunks.map((chunk) => chunk.code).join("\n");
    expect(rootRegistrationSource).toContain("`/sw.js`,{scope:`/`");
    expect(rootRegistrationSource).not.toMatch(/[`'"]\/\/(?:sw|registerSW)\.js/);
    expect(rootRegistrationSource).not.toContain("scope:`//`");
    const rootHtml = fs.readFileSync(path.join(root, "dist", "public", "index.html"), "utf8");
    expect(rootHtml).not.toMatch(/(?:src|href)="\/\/[^"]+"/);

    // Verify the exact same generated registration contract under the
    // artifact's non-root deployment path. This uses a second real build so
    // plugin output, not a mocked config object, remains the source of truth.
    const nestedNodeEnv = process.env.NODE_ENV;
    process.env.NODE_ENV = "production";
    process.env.BASE_PATH = "/bathyscan/";
    let nestedResult: RollupOutput | RollupOutput[];
    try {
      nestedResult = (await build({
        root,
        configFile: path.resolve(root, "vite.config.ts"),
        mode: "production",
        logLevel: "error",
      ...
      if (nestedNodeEnv === undefined) delete process.env.NODE_ENV;
      else process.env.NODE_ENV = nestedNodeEnv;
      if (prevBasePath === undefined) delete process.env.BASE_PATH;
      else process.env.BASE_PATH = prevBasePath;
    }
    const nestedOutputs = (Array.isArray(nestedResult) ? nestedResult : [nestedResult])
      .flatMap((output) => output.output)
      .filter((output) => output.type === "chunk");
    const nestedRegistrationSource = nestedOutputs
      .map((output) => (output.type === "chunk" ? output.code : ""))
      .join("\n");
    expect(nestedRegistrationSource).toContain(
      "`/bathyscan/sw.js`,{scope:`/bathyscan/`",
    );
    expect(nestedRegistrationSource).not.toMatch(/[`'"]\/\/(?:sw|registerSW)\.js/);
    expect(nestedRegistrationSource).not.toContain("scope:`//`");
    const nestedHtml = fs.readFileSync(path.join(root, "dist", "public", "index.html"), "utf8");
    expect(nestedHtml).not.toMatch(/(?:src|href)="\/\/[^"]+"/);
