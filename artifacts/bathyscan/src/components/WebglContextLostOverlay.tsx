...
export const WebglContextLostOverlay: React.FC = () => {
  ...
  if (contextPermanentlyLost) {
    return (
      ...
      >
        <div className="bg-[#0f172a] border border-red-700 text-red-300 font-mono text-[21px] tracking-wide px-8 py-6 shadow-2xl max-w-sm text-center space-y-4">
          <p id="webgl-fatal-title" className="text-[24px] font-bold uppercase tracking-widest text-red-400">
            Graphics context lost
          </p>
          <p className="text-[18px] text-red-200/80">
            The graphics context could not be restored after multiple attempts.
            Please reload the page to continue.
          </p>
          <button
            onClick={() => window.location.reload()}
            className="mt-2 px-5 py-2 bg-red-800 hover:bg-red-700 text-white font-mono text-[18px] uppercase tracking-widest border border-red-600"
          >
            Reload page
          </button>
        </div>
  ...
  return (
    ...
    >
      <div className="pointer-events-auto bg-[#020818]/90 border border-amber-700 text-amber-300 font-mono text-[18px] tracking-widest uppercase px-4 py-2 shadow-lg">
