...
export const getAdminSkillFailureGateUrl = () => {




  return `/api/admin/skill/failure-gate`
}

/**
 * Serves the failure-gate-skill.zip from disk with Cache-Control: no-store so the browser always downloads the latest copy regenerated by post-merge.sh. Restricted to admin users.

 * @summary Download the failure-gate skill zip with no-cache headers
 */
export const adminSkillFailureGate = async ( options?: RequestInit): Promise<Blob> => {

  return customFetch<Blob>(getAdminSkillFailureGateUrl(),
  {
    ...options,
    method: 'GET'
...
export const getAdminSkillFailureGateQueryKey = () => {
    return [
    `/api/admin/skill/failure-gate`
    ] as const;
    }


export const getAdminSkillFailureGateQueryOptions = <TData = Awaited<ReturnType<typeof adminSkillFailureGate>>, TError = ErrorType<ApiError>>( options?: { query?:UseQueryOptions<Awaited<ReturnType<typeof adminSkillFailureGate>>, TError, TData>, request?: SecondParameter<typeof customFetch>}
) => {

const {query: queryOptions, request: requestOptions} = options ?? {};

  const queryKey =  queryOptions?.queryKey ?? getAdminSkillFailureGateQueryKey();



    const queryFn: QueryFunction<Awaited<ReturnType<typeof adminSkillFailureGate>>> = ({ signal }) => adminSkillFailureGate({ signal, ...requestOptions });





   return  { queryKey, queryFn, ...queryOptions} as UseQueryOptions<Awaited<ReturnType<typeof adminSkillFailureGate>>, TError, TData> & { queryKey: QueryKey }
}

export type AdminSkillFailureGateQueryResult = NonNullable<Awaited<ReturnType<typeof adminSkillFailureGate>>>
export type AdminSkillFailureGateQueryError = ErrorType<ApiError>
...
export function useAdminSkillFailureGate<TData = Awaited<ReturnType<typeof adminSkillFailureGate>>, TError = ErrorType<ApiError>>(
  options?: { query?:UseQueryOptions<Awaited<ReturnType<typeof adminSkillFailureGate>>, TError, TData>, request?: SecondParameter<typeof customFetch>}

 ):  UseQueryResult<TData, TError> & { queryKey: QueryKey } {

  const queryOptions = getAdminSkillFailureGateQueryOptions(options)
