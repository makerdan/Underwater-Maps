...
export const GetDatasetsIdTerrainResponse = zod.object({
  ...
  "dataSource": zod.enum(['ncei', 'gebco', 'twdb', 'usace', 'usgs-3dep', 'usgs-sciencebase', 'noaa-great-lakes', 'nysdec', 'mn-dnr']).optional().describe('Which upstream data service produced this grid (bathymetry primary).\nncei            — NCEI Bag Mosaic WCS (high-resolution multibeam survey)\ngebco           — GEBCO 2024 WCS (~400 m global grid)\ntwdb            — TWDB Reservoir Volumetric & Sedimentation Survey\nusace           — USACE hydrographic survey\nusgs-3dep       — USGS 3DEP best-available DEM (pre-impoundment basin + topography)\nusgs-sciencebase — USGS ScienceBase hosted survey data\nnoaa-great-lakes — NOAA Great Lakes DEM mosaic\nnysdec          — NYSDEC Finger Lakes bathymetric contours (depth polygons)\nmn-dnr          — MN DNR lake bathymetric contour lines\n'),
  "bathymetrySource": zod.enum(['ncei', 'gebco', 'twdb', 'usace', 'usgs-3dep', 'usgs-sciencebase', 'noaa-great-lakes', 'nysdec', 'mn-dnr']).optional().describe('Per-layer provenance for the below-water (bathymetry) layer when it has a different source than topography (e.g. inland reservoirs).'),
  "topographySource": zod.enum(['ncei', 'gebco', 'twdb', 'usace', 'usgs-3dep', 'usgs-sciencebase', 'noaa-great-lakes', 'nysdec', 'mn-dnr']).optional().describe('Per-layer provenance for the above-water (topography) layer when it has a different source than bathymetry.'),
...
export const GetUserCollectionsResponseItem = zod.object({
  ...
  "bgOpacity": zod.number().min(getUserCollectionsResponseSpecialMetaBgOpacityMin).max(getUserCollectionsResponseSpecialMetaBgOpacityMax).describe('Background image opacity (default 0.5)'),
  "bgGeoAnchors": zod.array(zod.object({
  "lon": zod.number().min(getUserCollectionsResponseSpecialMetaBgGeoAnchorsItemLonMin).max(getUserCollectionsResponseSpecialMetaBgGeoAnchorsItemLonMax),
  "lat": zod.number().min(getUserCollectionsResponseSpecialMetaBgGeoAnchorsItemLatMin).max(getUserCollectionsResponseSpecialMetaBgGeoAnchorsItemLatMax),
  "imgX": zod.number().min(getUserCollectionsResponseSpecialMetaBgGeoAnchorsItemImgXMin),
  "imgY": zod.number().min(getUserCollectionsResponseSpecialMetaBgGeoAnchorsItemImgYMin)
}).describe('A control point mapping an image pixel to a geographic coordinate')).min(getUserCollectionsResponseSpecialMetaBgGeoAnchorsMin).max(getUserCollectionsResponseSpecialMetaBgGeoAnchorsMax).nullable().describe('Exactly two control points once registered; null until then'),
...
export const PostUserCollectionsBody = zod.object({
  "name": zod.string(),
  "collectionKind": zod.enum(['standard', 'special']).optional().describe('Defaults to standard; special collections are created with empty puzzle metadata')
})
...
export const PatchUserCollectionsIdRenameResponse = zod.object({
  ...
  "specialMeta": zod.object({
  "bgImageKey": zod.string().nullable().describe('Server storage key for the reference background image (null = none)'),
  "bgOpacity": zod.number().min(patchUserCollectionsIdRenameResponseSpecialMetaBgOpacityMin).max(patchUserCollectionsIdRenameResponseSpecialMetaBgOpacityMax).describe('Background image opacity (default 0.5)'),
  "bgGeoAnchors": zod.array(zod.object({
  "lon": zod.number().min(patchUserCollectionsIdRenameResponseSpecialMetaBgGeoAnchorsItemLonMin).max(patchUserCollectionsIdRenameResponseSpecialMetaBgGeoAnchorsItemLonMax),
  "lat": zod.number().min(patchUserCollectionsIdRenameResponseSpecialMetaBgGeoAnchorsItemLatMin).max(patchUserCollectionsIdRenameResponseSpecialMetaBgGeoAnchorsItemLatMax),
  "imgX": zod.number().min(patchUserCollectionsIdRenameResponseSpecialMetaBgGeoAnchorsItemImgXMin),
  "imgY": zod.number().min(patchUserCollectionsIdRenameResponseSpecialMetaBgGeoAnchorsItemImgYMin)
}).describe('A control point mapping an image pixel to a geographic coordinate')).min(patchUserCollectionsIdRenameResponseSpecialMetaBgGeoAnchorsMin).max(patchUserCollectionsIdRenameResponseSpecialMetaBgGeoAnchorsMax).nullable().describe('Exactly two control points once registered; null until then'),
  "layoutRevisions": zod.array(zod.object({
  "id": zod.string(),
  "name": zod.string(),
  "savedAt": zod.coerce.date(),
...
export const PostMarkersBody = zod.object({
  ...
  "depth": zod.number(),
  "type": zod.enum(['fish', 'shipwreck', 'coral', 'vent', 'custom', 'depth_pole', 'log', 'vegetation', 'sample', 'bass', 'trout', 'pike', 'walleye', 'crayfish', 'salmon', 'tuna', 'halibut', 'shark', 'swordfish', 'rockfish', 'cod', 'mahi_mahi', 'grouper', 'snapper', 'crab', 'lobster', 'shrimp', 'krill', 'jellyfish', 'octopus', 'squid', 'sea_urchin', 'starfish', 'sea_turtle', 'school_herring', 'school_sardine', 'school_mackerel', 'school_tuna', 'school_anchovy', 'catfish', 'crappie', 'bluegill', 'sunfish', 'carp', 'yellow_perch', 'muskie', 'largemouth_bass', 'smallmouth_bass', 'channel_catfish', 'freshwater_shrimp', 'freshwater_crab', 'snapping_turtle', 'bullfrog', 'beaver_dam', 'lily_pad', 'cattail', 'reed_bed', 'submerged_grass', 'spring', 'school_perch', 'school_bluegill', 'school_bass', 'school_crappie', 'school_carp', 'sand_bass', 'lake_trout', 'perch', 'rainbow_trout', 'silver_salmon', 'chinook_salmon', 'pink_salmon', 'turbot', 'black_rockfish', 'yelloweye_rockfish', 'dog_shark', 'dungeness_crab', 'prawn_shrimp', 'school_salmon', 'school_rockfish', 'lingcod', 'sole', 'multiple_logs', 'multiple_fish', 'submerged_rock', 'land', 'red_light', 'green_light', 'red_buoy', 'green_buoy', 'rock', 'clam', 'clam_beach', 'cool_rocks', 'rock_beach', 'brushpile', 'anchorage', 'hazard_rock', 'marina', 'boat_ramp', 'fuel_dock', 'diver_down', 'no_anchor', 'channel_marker', 'daymark']).default(postMarkersBodyTypeDefault),
...
export const patchMarkersIdBodyLabelMax = 200;
...
export const getSettingsResponseBandColorsMin = 2;
export const getSettingsResponseBandColorsMax = 16;

export const getSettingsResponseBandBoundariesItemMin = 0;
export const getSettingsResponseBandBoundariesItemMax = 36000;

export const getSettingsResponseBandBoundariesDefault = [0, 50, 100, 150, 200, 250, 300, 350, 450, 600, 2000];
export const getSettingsResponseBandBoundariesMin = 3;
...
export const GetSettingsResponse = zod.object({
  ...
  "bandColors": zod.array(zod.string().regex(getSettingsResponseBandColorsItemRegExp)).min(getSettingsResponseBandColorsMin).max(getSettingsResponseBandColorsMax).default(getSettingsResponseBandColorsDefault).describe('Per-band hex colours for the Ocean\/Custom depth palette. Variable length (2-16 bands), one entry per depth band. Must always be exactly one entry shorter than bandBoundaries. Persisted server-side so the palette is consistent across devices.'),
  "bandBoundaries": zod.array(zod.number().min(getSettingsResponseBandBoundariesItemMin).max(getSettingsResponseBandBoundariesItemMax)).min(getSettingsResponseBandBoundariesMin).max(getSettingsResponseBandBoundariesMax).default(getSettingsResponseBandBoundariesDefault).describe('Depth band boundaries in feet. Variable length (bandColors.length + 1 entries, 3-17), strictly increasing integers, first must be 0. Every other value including the last is user-editable up to 36000 ft. Persisted server-side so customised boundaries survive across devices.').superRefine((arr, ctx) => {
  if (arr[0] !== 0) {
  ...
  "ty": zod.number(),
  "angleDeg": zod.number(),
  "flipH": zod.boolean().optional().describe('Horizontal mirror applied to this tile. Absent = false (backward compat with pre-flip layouts).'),
  "flipV": zod.boolean().optional().describe('Vertical mirror applied to this tile. Absent = false (backward compat with pre-flip layouts).')
  ...
  "visibleMarkerTypes": zod.array(zod.enum(['fish', 'shipwreck', 'coral', 'vent', 'custom', 'depth_pole', 'log', 'vegetation', 'sample', 'bass', 'trout', 'pike', 'walleye', 'crayfish', 'salmon', 'tuna', 'halibut', 'shark', 'swordfish', 'rockfish', 'cod', 'mahi_mahi', 'grouper', 'snapper', 'crab', 'lobster', 'shrimp', 'krill', 'jellyfish', 'octopus', 'squid', 'sea_urchin', 'starfish', 'sea_turtle', 'school_herring', 'school_sardine', 'school_mackerel', 'school_tuna', 'school_anchovy', 'catfish', 'crappie', 'bluegill', 'sunfish', 'carp', 'yellow_perch', 'muskie', 'largemouth_bass', 'smallmouth_bass', 'channel_catfish', 'freshwater_shrimp', 'freshwater_crab', 'snapping_turtle', 'bullfrog', 'beaver_dam', 'lily_pad', 'cattail', 'reed_bed', 'submerged_grass', 'spring', 'school_perch', 'school_bluegill', 'school_bass', 'school_crappie', 'school_carp', 'sand_bass', 'lake_trout', 'perch', 'rainbow_trout', 'silver_salmon', 'chinook_salmon', 'pink_salmon', 'turbot', 'black_rockfish', 'yelloweye_rockfish', 'dog_shark', 'dungeness_crab', 'prawn_shrimp', 'school_salmon', 'school_rockfish', 'lingcod', 'sole', 'multiple_logs', 'multiple_fish', 'submerged_rock', 'land', 'red_light', 'green_light', 'red_buoy', 'green_buoy', 'rock', 'clam', 'clam_beach', 'cool_rocks', 'rock_beach', 'brushpile', 'anchorage', 'hazard_rock', 'marina', 'boat_ramp', 'fuel_dock', 'diver_down', 'no_anchor', 'channel_marker', 'daymark'])).default([`fish`, `shipwreck`, `coral`, `vent`, `custom`, `depth_pole`, `log`, `vegetation`, `sample`, `bass`, `trout`, `pike`, `walleye`, `crayfish`, `salmon`, `tuna`, `halibut`, `shark`, `swordfish`, `rockfish`, `cod`, `mahi_mahi`, `grouper`, `snapper`, `crab`, `lobster`, `shrimp`, `krill`, `jellyfish`, `octopus`, `squid`, `sea_urchin`, `starfish`, `sea_turtle`, `school_herring`, `school_sardine`, `school_mackerel`, `school_tuna`, `school_anchovy`, `catfish`, `crappie`, `bluegill`, `sunfish`, `carp`, `yellow_perch`, `muskie`, `largemouth_bass`, `smallmouth_bass`, `channel_catfish`, `freshwater_shrimp`, `freshwater_crab`, `snapping_turtle`, `bullfrog`, `beaver_dam`, `lily_pad`, `cattail`, `reed_bed`, `submerged_grass`, `spring`, `school_perch`, `school_bluegill`, `school_bass`, `school_crappie`, `school_carp`, `sand_bass`, `lake_trout`, `perch`, `rainbow_trout`, `silver_salmon`, `chinook_salmon`, `pink_salmon`, `turbot`, `black_rockfish`, `yelloweye_rockfish`, `dog_shark`, `dungeness_crab`, `prawn_shrimp`, `school_salmon`, `school_rockfish`, `lingcod`, `sole`, `multiple_logs`, `multiple_fish`, `submerged_rock`, `land`, `red_light`, `green_light`, `red_buoy`, `green_buoy`, `rock`, `clam`, `clam_beach`, `cool_rocks`, `rock_beach`, `brushpile`, `anchorage`, `hazard_rock`, `marina`, `boat_ramp`, `fuel_dock`, `diver_down`, `no_anchor`, `channel_marker`, `daymark`]),
  ...
  "defaultMarkerType": zod.enum(['fish', 'shipwreck', 'coral', 'vent', 'custom', 'depth_pole', 'log', 'vegetation', 'sample', 'bass', 'trout', 'pike', 'walleye', 'crayfish', 'salmon', 'tuna', 'halibut', 'shark', 'swordfish', 'rockfish', 'cod', 'mahi_mahi', 'grouper', 'snapper', 'crab', 'lobster', 'shrimp', 'krill', 'jellyfish', 'octopus', 'squid', 'sea_urchin', 'starfish', 'sea_turtle', 'school_herring', 'school_sardine', 'school_mackerel', 'school_tuna', 'school_anchovy', 'catfish', 'crappie', 'bluegill', 'sunfish', 'carp', 'yellow_perch', 'muskie', 'largemouth_bass', 'smallmouth_bass', 'channel_catfish', 'freshwater_shrimp', 'freshwater_crab', 'snapping_turtle', 'bullfrog', 'beaver_dam', 'lily_pad', 'cattail', 'reed_bed', 'submerged_grass', 'spring', 'school_perch', 'school_bluegill', 'school_bass', 'school_crappie', 'school_carp', 'sand_bass', 'lake_trout', 'perch', 'rainbow_trout', 'silver_salmon', 'chinook_salmon', 'pink_salmon', 'turbot', 'black_rockfish', 'yelloweye_rockfish', 'dog_shark', 'dungeness_crab', 'prawn_shrimp', 'school_salmon', 'school_rockfish', 'lingcod', 'sole', 'multiple_logs', 'multiple_fish', 'submerged_rock', 'land', 'red_light', 'green_light', 'red_buoy', 'green_buoy', 'rock', 'clam', 'clam_beach', 'cool_rocks', 'rock_beach', 'brushpile', 'anchorage', 'hazard_rock', 'marina', 'boat_ramp', 'fuel_dock', 'diver_down', 'no_anchor', 'channel_marker', 'daymark']).default(getSettingsResponseDefaultMarkerTypeDefault),
...
export const putSettingsBodyBandColorsMin = 2;
export const putSettingsBodyBandColorsMax = 16;

export const putSettingsBodyBandBoundariesItemMin = 0;
export const putSettingsBodyBandBoundariesItemMax = 36000;

export const putSettingsBodyBandBoundariesDefault = [0, 50, 100, 150, 200, 250, 300, 350, 450, 600, 2000];
export const putSettingsBodyBandBoundariesMin = 3;
...
export const PutSettingsBody = zod.object({
  ...
  "defaultMarkerType": zod.enum(['fish', 'shipwreck', 'coral', 'vent', 'custom', 'depth_pole', 'log', 'vegetation', 'sample', 'bass', 'trout', 'pike', 'walleye', 'crayfish', 'salmon', 'tuna', 'halibut', 'shark', 'swordfish', 'rockfish', 'cod', 'mahi_mahi', 'grouper', 'snapper', 'crab', 'lobster', 'shrimp', 'krill', 'jellyfish', 'octopus', 'squid', 'sea_urchin', 'starfish', 'sea_turtle', 'school_herring', 'school_sardine', 'school_mackerel', 'school_tuna', 'school_anchovy', 'catfish', 'crappie', 'bluegill', 'sunfish', 'carp', 'yellow_perch', 'muskie', 'largemouth_bass', 'smallmouth_bass', 'channel_catfish', 'freshwater_shrimp', 'freshwater_crab', 'snapping_turtle', 'bullfrog', 'beaver_dam', 'lily_pad', 'cattail', 'reed_bed', 'submerged_grass', 'spring', 'school_perch', 'school_bluegill', 'school_bass', 'school_crappie', 'school_carp', 'sand_bass', 'lake_trout', 'perch', 'rainbow_trout', 'silver_salmon', 'chinook_salmon', 'pink_salmon', 'turbot', 'black_rockfish', 'yelloweye_rockfish', 'dog_shark', 'dungeness_crab', 'prawn_shrimp', 'school_salmon', 'school_rockfish', 'lingcod', 'sole', 'multiple_logs', 'multiple_fish', 'submerged_rock', 'land', 'red_light', 'green_light', 'red_buoy', 'green_buoy', 'rock', 'clam', 'clam_beach', 'cool_rocks', 'rock_beach', 'brushpile', 'anchorage', 'hazard_rock', 'marina', 'boat_ramp', 'fuel_dock', 'diver_down', 'no_anchor', 'channel_marker', 'daymark']).default(putSettingsBodyDefaultMarkerTypeDefault),
...
export const putSettingsResponseBandColorsMin = 2;
export const putSettingsResponseBandColorsMax = 16;

export const putSettingsResponseBandBoundariesItemMin = 0;
export const putSettingsResponseBandBoundariesItemMax = 36000;

export const putSettingsResponseBandBoundariesDefault = [0, 50, 100, 150, 200, 250, 300, 350, 450, 600, 2000];
export const putSettingsResponseBandBoundariesMin = 3;
...
export const PutSettingsResponse = zod.object({
  ...
  "bandBoundaries": zod.array(zod.number().min(putSettingsResponseBandBoundariesItemMin).max(putSettingsResponseBandBoundariesItemMax)).min(putSettingsResponseBandBoundariesMin).max(putSettingsResponseBandBoundariesMax).default(putSettingsResponseBandBoundariesDefault).describe('Depth band boundaries in feet. Variable length (bandColors.length + 1 entries, 3-17), strictly increasing integers, first must be 0. Every other value including the last is user-editable up to 36000 ft. Persisted server-side so customised boundaries survive across devices.').superRefine((arr, ctx) => {
  if (arr[0] !== 0) {
    ctx.addIssue({ code: zod.ZodIssueCode.custom, message: "bandBoundaries first element must be 0", path: [0] });
  }
  for (let i = 1; i < arr.length; i++) {
  ...
  "defaultMarkerType": zod.enum(['fish', 'shipwreck', 'coral', 'vent', 'custom', 'depth_pole', 'log', 'vegetation', 'sample', 'bass', 'trout', 'pike', 'walleye', 'crayfish', 'salmon', 'tuna', 'halibut', 'shark', 'swordfish', 'rockfish', 'cod', 'mahi_mahi', 'grouper', 'snapper', 'crab', 'lobster', 'shrimp', 'krill', 'jellyfish', 'octopus', 'squid', 'sea_urchin', 'starfish', 'sea_turtle', 'school_herring', 'school_sardine', 'school_mackerel', 'school_tuna', 'school_anchovy', 'catfish', 'crappie', 'bluegill', 'sunfish', 'carp', 'yellow_perch', 'muskie', 'largemouth_bass', 'smallmouth_bass', 'channel_catfish', 'freshwater_shrimp', 'freshwater_crab', 'snapping_turtle', 'bullfrog', 'beaver_dam', 'lily_pad', 'cattail', 'reed_bed', 'submerged_grass', 'spring', 'school_perch', 'school_bluegill', 'school_bass', 'school_crappie', 'school_carp', 'sand_bass', 'lake_trout', 'perch', 'rainbow_trout', 'silver_salmon', 'chinook_salmon', 'pink_salmon', 'turbot', 'black_rockfish', 'yelloweye_rockfish', 'dog_shark', 'dungeness_crab', 'prawn_shrimp', 'school_salmon', 'school_rockfish', 'lingcod', 'sole', 'multiple_logs', 'multiple_fish', 'submerged_rock', 'land', 'red_light', 'green_light', 'red_buoy', 'green_buoy', 'rock', 'clam', 'clam_beach', 'cool_rocks', 'rock_beach', 'brushpile', 'anchorage', 'hazard_rock', 'marina', 'boat_ramp', 'fuel_dock', 'diver_down', 'no_anchor', 'channel_marker', 'daymark']).default(putSettingsResponseDefaultMarkerTypeDefault),
 ...
 */
export const getNceiSearchQueryBroadDefault = false;
...
export const GetNceiSearchQueryParams = zod.object({
  ...
  "bbox": zod.coerce.string().optional().describe('Spatial filter as \"minLon,minLat,maxLon,maxLat\"'),
  "broad": zod.coerce.boolean().default(getNceiSearchQueryBroadDefault).describe('When true and q is empty, skip the implicit \"bathymetry\" keyword\ndefault so the search returns all NCEI record types in the area\n(used by the \"Other data in this area\" reference listing).\n'),
...
export const AdminListUsersQueryParams = zod.object({
  ...
  "cursor": zod.coerce.string().optional().describe('Clerk user ID of the last row from the previous page.')
})
...
export const AdminBanUserBody = zod.object({
  "note": zod.string().max(adminBanUserBodyNoteMax).optional()
})
