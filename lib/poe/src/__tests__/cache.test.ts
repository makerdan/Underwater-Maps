...
describe("PoeCache", () => {
  ...
  it("evicts oldest entry when over capacity", () => {
    ...
    for (let i = 0; i < 101; i++) {
      (smallCache as unknown as { store: Map<string, unknown> }).store.set(
        `overflow-key-${i}`,
        { value: `v${i}`, expiresAt: Date.now() + 60_000 },
      );
    }
    expect(
      (smallCache as unknown as { store: Map<string, unknown> }).store.size,
