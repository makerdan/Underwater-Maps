...
describe("poeComplete", () => {
  beforeEach(() => {
    vi.mocked(getPoeClient).mockReturnValue(makeMockClient() as unknown as ReturnType<typeof getPoeClient>);
  });
  ...
  it("parses tool_calls when present", async () => {
    ...
    vi.mocked(getPoeClient).mockReturnValue(mockClient as unknown as ReturnType<typeof getPoeClient>);
