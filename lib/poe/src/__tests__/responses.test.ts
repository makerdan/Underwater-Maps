...
function makeMockClient(outputText: string, usage = { input_tokens: 100, output_tokens: 50 }) {
  return {
    responses: {
...
describe("poeRespond", () => {
  beforeEach(() => {
    vi.mocked(getPoeClient).mockReturnValue(
      makeMockClient("Hello from Poe") as unknown as ReturnType<typeof getPoeClient>,
    );
  });
  ...
  it("accepts valid JSON when jsonSchema is set", async () => {
    vi.mocked(getPoeClient).mockReturnValue(
      makeMockClient(JSON.stringify({ zones: ["sandy_shelf"] })) as unknown as ReturnType<typeof getPoeClient>,
    );
    const result = await poeRespond({
      model: "Claude-Sonnet-4.6",
      input: "Classify",
  ...
  it("throws ZoneParseError when Zod validation fails", async () => {
    const zones: string[] = ["bad_zone"];
    vi.mocked(getPoeClient).mockReturnValue(
      makeMockClient(JSON.stringify({ zones })) as unknown as ReturnType<typeof getPoeClient>,
    );
  ...
  it("passes when Zod validation succeeds", async () => {
    vi.mocked(getPoeClient).mockReturnValue(
      makeMockClient(JSON.stringify({ zones: ["sandy_shelf", "trench_wall"] })) as unknown as ReturnType<typeof getPoeClient>,
    );
