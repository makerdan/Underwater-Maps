...
export function buildVisionInput(prompt, imageDataUrl) {
    return [
        { type: "input_text", text: prompt },
        { type: "input_image", image_url: imageDataUrl },
    ];
}
export function buildMultiModalMessages(systemPrompt, userText, imageDataUrl) {
    return [
        {
            role: "system",
...
export function depthGridToBase64Png(grid, targetSize = 256) {
    ...
    for (let y = 0; y < size; y++) {
        for (let x = 0; x < size; x++) {
            ...
            const depth = typeof depths[srcIdx] === "number" ? depths[srcIdx] : 0;
...
function encodePngGreyscale(rgba, width, height) {
