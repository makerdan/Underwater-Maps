...
export function getValidationSteps(logPrefix) {
  ...
  return [
    ...
    // no resource: strict Failure Gate lint for TASK_PLAN_FILE only. With no
    // task context it skips; archive inspection is explicit maintenance via
    // check-failure-gate.mjs --archive and never blocks managed tiers.
    ...
    // no resource: exercises the real pre-commit hook's task-plan and docs paths
    { name: "check:pre-commit-self-test", resource: null, cmd: "pnpm run check:pre-commit-self-test", tiers: FAST },
